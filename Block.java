...
public class Block {
    ...
    public String getNounce() {
        String temp_nounce = "";
        String hashAllData = encrypt(String.valueOf(this.index) +this.data + this.timestamp + this.prevHash + this.hash);
        int i = 0;
        while (i<5) {
            temp_nounce = encrypt(temp_nounce + hashAllData);
            char[] nounceArry = temp_nounce.toCharArray();
            if (nounceArry[i] == '0') {
                i++;
            }
        }
